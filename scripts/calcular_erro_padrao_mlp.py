...
for cripto in df["Criptomoeda"].unique():
    ...
    for modelo in df_cripto["Modelo"].unique():
        previsoes_por_modelo[modelo] = df_cripto[df_cripto["Modelo"] == modelo]["Valor Previsto"].values

    # Calcular RMSEs (exceto MLP)
    y_real = df_cripto[df_cripto["Modelo"] == "MLP"]["Valor Real"].values  # mesmo real para todos
    rmse_modelos = {
...
df_erro_padrao.sort_values(by="Erro Padrão (MLP vs Melhor)", ascending=False, inplace=True)
