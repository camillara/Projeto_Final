...
def test_calcular_erro_padrao(tmp_path):
    ...
    assert set(df_saida.columns) == {
        "Criptomoeda",
        "Modelo",
        "RMSE",
    }, "Colunas incorretas no arquivo de saída"

    # Verificar se RMSEs estão corretos
    for _, row in df_saida.iterrows():
        df_filtrado = df_mock[
            (df_mock["Criptomoeda"] == row["Criptomoeda"])
            & (df_mock["Modelo"] == row["Modelo"])
        ]
        esperado = np.sqrt(
            np.mean((df_filtrado["Valor Real"] - df_filtrado["Valor Previsto"]) ** 2)
        )
        assert np.isclose(
            row["RMSE"], esperado, atol=1e-6
        ), f"RMSE incorreto para {row['Criptomoeda']} - {row['Modelo']}"
