...
def test_simulacao_lucro_calculado_corretamente() -> None:
    ...
    lucro, resultado = simular_estrategia_investimento(
        df, previsoes, threshold=0.01, verbose=False
    )
    ...
    assert (
        "RetornoPercentual" in resultado.columns
    ), "Coluna 'RetornoPercentual' ausente"
...
def test_simulacao_sem_compras_quando_previsao_baixa() -> None:
    ...
    lucro, resultado = simular_estrategia_investimento(
        df, previsoes, threshold=0.05, verbose=False
    )

    assert lucro == 0, "Lucro deveria ser zero quando nenhuma compra é realizada"
    assert (
        resultado.empty
        or "Lucro" not in resultado.columns
        or resultado["Lucro"].sum() == 0
