...
def test_adicionar_features_basicas() -> None:
    ...
    colunas_esperadas = [
        "Retorno", "TendenciaAlta", "Amplitude", "Variacao_Alta_Abertura",
        "Variacao_Abertura_Baixa", "Razao_Alta_Baixa", "Razao_Retorno_Desvio",
        "MediaMovel_7d", "MediaMovel_14d", "MediaMovel_30d",
        "DesvioPadrao_7d", "DesvioPadrao_14d",
        "MediaVolume_7d", "MediaVolume_14d",
        "Retorno_Anterior_1d", "Retorno_Anterior_2d",
        "DiaSemana", "Mes", "FimDeSemana"
    ]

    for coluna in colunas_esperadas:
        assert coluna in df_resultado.columns, f"Coluna esperada '{coluna}' não encontrada no resultado."
    ...
    assert df_resultado.isna().sum().sum() == 0, "Ainda existem valores NaN no DataFrame final."
